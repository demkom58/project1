...
public class Behavior<T> : IBehaviorControl<T> where T : ILivingEntity
{
    ...
    public bool TryStart(IWorld level, T entity, long updateNumber)
    {
        if (HasRequiredMemories(entity) && CheckExtraStartConditions(level, entity))
        {
            ...
            _endTimestamp = updateNumber + randomizedDuration;

            Start(level, entity, updateNumber);
            return true;
        }
    ...
    {
        if (!TimedOut(updateNumber) && CanStillUse(level, entity, updateNumber))
            Tick(level, entity, updateNumber);
        else
            DoStop(level, entity, updateNumber);
    }

    public void DoStop(IWorld level, T entity, long updateNumber)
    {
        Status = BehaviorStatus.Stopped;
        Stop(level, entity, updateNumber);
    }
